...
(function() {
  ...
  function connect() {
    // 自动检测 WebSocket 地址（与 HTTP 同端口）
    const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
    // WebSocket 与 HTTP 共用端口
    const wsPort = parseInt(location.port || '80');
  ...
  function updateCursor(x, y) {
    ...
    cursorIndicator.style.left = (x - rect.left) + 'px';
    cursorIndicator.style.top = (y - rect.top) + 'px';
